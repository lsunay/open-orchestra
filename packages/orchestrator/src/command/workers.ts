...
import { getOrchestratorContext, type ToolContext } from "./state";

type WorkerTools = {
  listWorkers: ToolDefinition;
  askWorker: ToolDefinition;
  askWorkerAsync: ToolDefinition;
  awaitWorkerJob: ToolDefinition;
  getWorkerJob: ToolDefinition;
  listWorkerJobs: ToolDefinition;
  getWorkerInfo: ToolDefinition;
  spawnNewWorker: ToolDefinition;
  ensureWorkers: ToolDefinition;
  stopWorkerTool: ToolDefinition;
...
export function createWorkerTools(context: OrchestratorContext): WorkerTools {
  ...
  const listWorkers: ToolDefinition = tool({
  ...
  const askWorker: ToolDefinition = tool({
...
- explorer: For quick codebase searches`,
  ...
  const listWorkerJobs: ToolDefinition = tool({
  ...
  const getWorkerInfo: ToolDefinition = tool({
  ...
  const spawnNewWorker: ToolDefinition = tool({
...
You can also provide custom configuration to override defaults.`,
    ...
    async execute(args, ctx: ToolContext) {
      ...
      if (args.model) {
        const normalized = await normalizeModelInput(args.model, { client: context.client, directory: context.directory });
      ...
      try {
        ...
        if (args.showToast && client) {
          void client.tui
            .showToast({
              body: { message: `Worker "${profile.name}" spawned`, variant: "success" },
            })
            .catch(() => {});
        }
        ...
        if (args.showToast && client) {
          void client.tui
            .showToast({
              body: { message: `Worker spawn failed: ${message}`, variant: "error" },
            })
            .catch(() => {});
        }
  ...
  const ensureWorkers: ToolDefinition = tool({
  ...
  const stopWorkerTool: ToolDefinition = tool({
  ...
  const delegateTask: ToolDefinition = tool({
    ...
    async execute(args, ctx: ToolContext) {
      ...
      const result = await sendToWorker(targetId, args.task, {
        attachments: args.attachments,
        sessionId: ctx?.sessionID,
      });
  ...
  const findWorker: ToolDefinition = tool({
  ...
  const workerTrace: ToolDefinition = tool({
    description:
...
const defaultTools: WorkerTools = createWorkerTools(getOrchestratorContext());

export const listWorkers: ToolDefinition = defaultTools.listWorkers;
export const askWorker: ToolDefinition = defaultTools.askWorker;
export const askWorkerAsync: ToolDefinition = defaultTools.askWorkerAsync;
export const awaitWorkerJob: ToolDefinition = defaultTools.awaitWorkerJob;
export const getWorkerJob: ToolDefinition = defaultTools.getWorkerJob;
export const listWorkerJobs: ToolDefinition = defaultTools.listWorkerJobs;
export const getWorkerInfo: ToolDefinition = defaultTools.getWorkerInfo;
export const spawnNewWorker: ToolDefinition = defaultTools.spawnNewWorker;
export const ensureWorkers: ToolDefinition = defaultTools.ensureWorkers;
export const stopWorkerTool: ToolDefinition = defaultTools.stopWorkerTool;
